...
checkpoint_callback = ModelCheckpoint(
    filepath=config["trainer"]["checkpoint"],
    save_top_k=1,
    verbose=True,
    monitor="epoch",
    mode="max",
...
def train_model(
        lit_model: LightningModule, tensorboard_graph_name: str = None, max_epochs: int = 4,
):
    logger = False
    ...
    if os.path.exists(config["trainer"]["checkpoint"]):
        trainer = Trainer(
            resume_from_checkpoint=config["trainer"]["checkpoint"],
            checkpoint_callback=checkpoint_callback,
            callbacks=[
                SaveCheckpointAtEpochEnd(filepath=config["trainer"]["checkpoint"])
            ],
